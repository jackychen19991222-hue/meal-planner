...
function buildGroceryList(selectedRecipes, people, lang) {
  const map = {};
...
function runDeveloperTests() {
  const selected = [recipes[0], recipes[1]];
  const grocery = buildGroceryList(selected, 4, "zh");

  console.assert(grocery.vegetables.length === 2, "Test failed: vegetables should include tomato and scallion");
  console.assert(grocery.dairy[0].qty === 6, "Test failed: eggs should scale from 3 to 6 for 4 people");
  console.assert(grocery.meat[0].qty === 800, "Test failed: chicken should scale from 400g to 800g for 4 people");
  console.assert(translations.zh.appName === "一周菜单管家", "Test failed: Chinese should be the default product language");
}
              ...
              <div className="p-5">
                ...
                <div className="mt-5 space-y-5">
                  {Object.entries(groceryByCategory).map(([cat, items]) => (
                    <div key={cat}>
                      <h3 className="text-sm font-semibold text-neutral-300 mb-2">{t.categories[cat]}</h3>
